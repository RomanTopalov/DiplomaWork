var Test_topalovDiagramLayout = function(layoutContext, optLinkLength, initialTemp) {
...
Test_topalovDiagramLayout.PAD_FACTOR = 1.2;
//initial temperature factor - percent of ideal viewport dimension
Test_topalovDiagramLayout.INIT_TEMP_FACTOR = .25; 
//number of iterations to run
Test_topalovDiagramLayout.ITERATIONS = 200; 
 ...
 */
Test_topalovDiagramLayout.forceDirectedLayout = function(layoutContext)
{
  //pretend that the layout area is just big enough to fit all the nodes
  var maxBounds = Test_topalovDiagramLayout.getMaxNodeBounds(layoutContext);
  var nodeCount = layoutContext.getNodeCount();
  var area = nodeCount * (Test_topalovDiagramLayout.PAD_FACTOR * maxBounds.w) * (Test_topalovDiagramLayout.PAD_FACTOR * maxBounds.h);
  var initialTemp = Test_topalovDiagramLayout.INIT_TEMP_FACTOR * Math.sqrt(area);
  ...
  var layout = new Test_topalovDiagramLayout(layoutContext, optLinkLength, initialTemp);
  layout.layoutNodes();
  layout.layoutLinks(); 
 ...
 * Layout nodes
 */
Test_topalovDiagramLayout.prototype.layoutNodes = function () {
  this.initForceDirectedPositions();
  var iter = Test_topalovDiagramLayout.ITERATIONS; 
 ...
 */
Test_topalovDiagramLayout.prototype.runForceDirectedIteration = function(t)
{    
 ...
 */
Test_topalovDiagramLayout.prototype._addGravity = function(node) {
 ...
 */
Test_topalovDiagramLayout.prototype.initForceDirectedPositions = function() {
  var nodeCount = this._layoutContext.getNodeCount();
 ...
 */
Test_topalovDiagramLayout.prototype._vectorLength = function(p) {
 ...
 */
Test_topalovDiagramLayout.prototype._scaleVector = function(p, scale) {
 ...
 * @return {DvtDiagramPoint} resulting vector
 */
Test_topalovDiagramLayout.prototype._addVectors = function(p1, p2) {
 ...
 * @return {DvtDiagramPoint} resulting vector 
 */
Test_topalovDiagramLayout.prototype._subtractVectors = function(p1, p2) {
  return new DvtDiagramPoint(p1.x - p2.x, p1.y - p2.y);
};
 ...
 */
Test_topalovDiagramLayout.prototype._getNodeAtCurrentLevel = function(nodeId) {
  var node;
  do {
    if (!nodeId)
      return null;
 ...
 * Create links
 */
Test_topalovDiagramLayout.prototype.layoutLinks = function () {
 ...
 */
Test_topalovDiagramLayout.prototype.getEndpoints = function (link) {
 ...
 */
Test_topalovDiagramLayout.prototype._findLinkNodeIntersection = function (rect, startX, startY, endX, endY, connOffset) {
 ...
 */
Test_topalovDiagramLayout.prototype.positionNodeLabels = function () {
 ...
 */
Test_topalovDiagramLayout.prototype.positionLinkLabels = function (layoutContext) {
 ...
 */
Test_topalovDiagramLayout.getMaxNodeBounds = function (layoutContext) {
  var nodeCount = layoutContext.getNodeCount();
  var maxW = 0 , maxH = 0;
  for (var ni = 0;ni < nodeCount;ni++) {
